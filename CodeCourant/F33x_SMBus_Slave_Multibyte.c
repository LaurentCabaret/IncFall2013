...
#define STARTPOSLat 	48.764308
#define STARTPOSLong	2.288964
...
void main (void)
{
  ...
  /* for (i = 0; i < NUM_BYTES_RD; i++)
   ...
   {
	  theta += 0.04;
	  LatLong.AccesFlottants.Latitude = STARTPOSLat + 0.0015*cos(theta);
  	  LatLong.AccesFlottants.Longitude = STARTPOSLong + 0.002*sin(theta);
      ...
      if (SMB_DATA_IN[0]==CMD_GETLAT) { // GetLat
	      memcpy(SMB_DATA_OUT, LatLong.AccesOctets, 4);
	      // Copy the data from the input array to the output array
		  DataToSend = 7;          
  		  LED =1;
//		  48.765351,2.288409  --> Bat Ens
      ...
      if (SMB_DATA_IN[0]==CMD_GETLONG) { // GetLong
	      // Copy the data from the input array to the output array
		  memcpy(SMB_DATA_OUT, LatLong.AccesOctets+4, 4);
		  DataToSend = 7;
  		  LED =1;
	}
